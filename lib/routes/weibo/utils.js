...
const weiboUtils = {
    formatExtended: (ctx, status, params = {}, picsPrefixes = []) => {
        ...
        const mergedParams = {
            ...
            showEmojiForRetweet: fallback(params.showEmojiForRetweet, queryToBoolean(routeParams.showEmojiForRetweet), true),
        ...
        htmlNewLineUnreplaced = htmlNewLineUnreplaced.replace(/<span class='url-icon'><img.*?><\/span>/g, '');
        // 表情图标转换为文字
    ...
    formatVideo: (itemDesc, status, itemTitle = '') => {
